#!/usr/bin/env bash
# Full pipeline for the mackey_glass experiment: R-STDP (main) + surrogate/GRU baselines
# + linear baseline + results aggregation + Lava spec export.
# Extra CLI args are forwarded to every step, e.g.:
#   bash scripts/run_mackey_glass.sh quantization.enabled=true quantization.bits=8
set -e
cd "$(dirname "$0")/.."
CFG=configs/mackey_glass.yaml
...
echo ">> [5/5] done. Results in results/mackey_glass/"
