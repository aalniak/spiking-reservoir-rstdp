#!/usr/bin/env bash
# Full pipeline for the narma10 experiment: R-STDP (main) + surrogate/GRU baselines
# + linear baseline + results aggregation + Lava spec export.
# Extra CLI args are forwarded to every step, e.g.:
#   bash scripts/run_narma10.sh quantization.enabled=true quantization.bits=8
set -e
cd "$(dirname "$0")/.."
CFG=configs/narma10.yaml
...
echo ">> [5/5] done. Results in results/narma10/"
